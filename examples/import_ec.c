...
const char p256_pvtkey_file[] = "p256_pvtkey.pem";
...
int main(void) {
  ...
  FILE *fp = fopen(p256_pvtkey_file, "rb");
  assert(fp != NULL);
  ...
  assert(ret == true);
  assert(algorithm == YH_ALGO_EC_P256);
  ...
  yrc = yh_util_import_ec_key(session, &key_id, key_label, domain_five,
