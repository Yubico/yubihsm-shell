if($args.length -lt 3)
{
    echo "Usage: ./make_installer.ps1 <x86|x64> <VERSION> <WIX_PATH> <MERGE_MODULE_PATH> [<SIGNED_BINARIES_PATH>]"
    echo ""
    ...
    echo ""
    echo "   VERSION                Release version, tex 2.6.3"
...
light.exe fragment.wixobj "yubihsm-shell_$ARCH.wixobj" -ext WixUIExtension -ext WixUtilExtension -o "yubihsm-shell-$VERSION-$ARCH.msi"

#cleanup
rm fragment.wxs
rm fragment.wixobj
rm "yubihsm-shell_$ARCH.wixobj"
rm "yubihsm-shell-$VERSION-$ARCH.wixpdb"

if($ARCH -eq "x86")
{
    mv "yubihsm-shell-$VERSION-$ARCH.msi" "yubihsm-shell-$VERSION-win32.msi"
}
else
{
    mv "yubihsm-shell-$VERSION-$ARCH.msi" "yubihsm-shell-$VERSION-win64.msi"
}
