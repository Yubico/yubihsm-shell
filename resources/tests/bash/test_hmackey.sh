...
echo "====================== HMAC keys ===================== "
echo "------------- hmac-sha1"
test_with_resp ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a generate-hmac-key -i 0 -l hmackey -d 1,2,3 -c sign-hmac -A hmac-sha1" "   Generate key"
keyid=$(tail -1 resp.txt | awk '{print $4}')
test ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a get-object-info -i $keyid -t hmac-key" "   Get object info"
info=$("$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a get-object-info -i $keyid -t hmac-key 2> /dev/null)
...
test ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a delete-object -i $keyid -t hmac-key" "   Delete key"

echo "------------- hmac-sha256"
test_with_resp ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a generate-hmac-key -i 0 -l hmackey -d 1,2,3 -c sign-hmac -A hmac-sha256" "   Generate key"
keyid=$(tail -1 resp.txt | awk '{print $4}')
test ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a get-object-info -i $keyid -t hmac-key" "   Get object info"
info=$("$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a get-object-info -i $keyid -t hmac-key 2> /dev/null)
...
echo "------------- hmac-sha384"
test_with_resp ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a generate-hmac-key -i 0 -l hmackey -d 1,2,3 -c sign-hmac -A hmac-sha384" "   Generate key"
keyid=$(tail -1 resp.txt | awk '{print $4}')
test ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a get-object-info -i $keyid -t hmac-key" "   Get object info"
info=$("$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a get-object-info -i $keyid -t hmac-key 2> /dev/null)
...
echo "------------- hmac-sha512"
test_with_resp ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a generate-hmac-key -i 0 -l hmackey -d 1,2,3 -c sign-hmac -A hmac-sha512" "   Generate key"
keyid=$(tail -1 resp.txt | awk '{print $4}')
test ""$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a get-object-info -i $keyid -t hmac-key" "   Get object info"
info=$("$BIN" "$c_var" "$SPECIFIED_CONNECTOR" -p password -a get-object-info -i $keyid -t hmac-key 2> /dev/null)
